...
def generate_score_table(alphafold_dir, output_dir):
    beta_threshold = 0.3
    turn_threshold = 0.2
    ...
    res = pd.DataFrame(columns=["Filename", "Beta 1 Score", "Turn Score", "Beta 2 Score", "Plddt", "Plddt Target"])

    for sname, mname in get_full_runs(alphafold_dir):
        ...
        target_residues = set(target_beta_residues_1).union(target_turn_residues).union(target_beta_residues_2)
        ...
        beta_sum = sum([beta_template[ri] for ri in target_beta_residues_1 + target_beta_residues_2])
